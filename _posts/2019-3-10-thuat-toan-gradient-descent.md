...
- <i>Momentum gradient descent</i>: Biến thể này sử dụng một khái niệm được gọi là momentum để giúp tăng tốc độ hội tụ và tránh được các vùng cục bộ của hàm mất mát. Momentum gradient descent tính toán [gradient](https://vi.wikipedia.org/wiki/Gradient) không chỉ dựa trên [gradient](https://vi.wikipedia.org/wiki/Gradient) hiện tại mà còn dựa trên [gradient](https://vi.wikipedia.org/wiki/Gradient) trước đó để giảm [độ nhạy cảm của thuật toán](https://en.wikipedia.org/wiki/Output-sensitive_algorithm) với nhiễu và vùng lõm.

- <i>Nesterov accelerated gradient</i>: Biến thể này cải thiện Momentum gradient descent bằng cách sử dụng công thức truy hồi để tìm kiếm [gradient](https://vi.wikipedia.org/wiki/Gradient) gần với thực tế nhất có thể. Việc này giúp Nesterov accelerated gradient có thể tìm kiếm [gradient](https://vi.wikipedia.org/wiki/Gradient) tốt hơn và giảm [độ nhạy cảm của thuật toán](https://en.wikipedia.org/wiki/Output-sensitive_algorithm) với nhiễu.
