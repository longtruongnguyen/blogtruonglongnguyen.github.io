...
- [Neural Language Models](https://towardsdatascience.com/neural-language-models-32bec14d01dc): Các mô hình ngôn ngữ hiện đại sử dụng các kiến trúc mạng neural phức tạp hơn để học cấu trúc ngôn ngữ. Các kiến trúc để xây dựng mô hình ngôn ngữ phổ biến bao gồm LSTM (Long Short-Term Memory), GRU (Gated Recurrent Unit) và Transformer. Những mô hình này có khả năng học cả sự phụ thuộc dài hạn giữa các từ trong câu, giúp cho kết quả dự đoán xác suất của mô hình ngày càng chính xác. Đây là phương pháp phổ biến và hiệu quả nhất để xây dựng mô hình ngôn ngữ.
